...
{   
    ...
    {   
        ...
        return view('modules/components/suppliers/manage')->with(compact('menus', 'terms'));
    }
    ...
    {   
        ...
        return view('modules/components/suppliers/manage-inactive')->with(compact('menus'));
    }
    ...
    {   
        ...
        if ($rows > 0) {
            $data = array(
                'title' => 'Oh snap!',
                'text' => 'You cannot create a supplier with an existing code.',
                'type' => 'error',
                'field' => 'code',
                'class' => 'btn-danger'
            );
        ...
        $this->audit_logs('suppliers', $supplier->id, 'has inserted a new supplier.', Supplier::find($supplier->id), $timestamp, Auth::user()->id);
        
        $data = array(
            'title' => 'Well done!',
            'text' => 'The supplier has been successfully stored.',
            'type' => 'success',
            'class' => 'btn-brand'
        );
    ...
    {    
        ...
        $supplier->payment_terms_id = $request->payment_terms_id;
        ...
        if ($supplier->update()) {
            $this->audit_logs('suppliers', $id, 'has modified a supplier.', Supplier::find($id), $timestamp, Auth::user()->id);
            $data = array(
                'title' => 'Well done!',
                'text' => 'The supplier has been successfully modified.',
                'type' => 'success',
                'class' => 'btn-brand'
            );
            echo json_encode( $data ); exit();
    ...
    {   
        ...
        $msg .= '<table class="table align-middle table-row-dashed fs-6 gy-5" id="supplierTable">';
        $msg .= '<thead>';
            ...
            $msg .= '<input class="form-check-input" type="checkbox" data-kt-check="true" data-kt-check-target="#kt_supplier_table .form-check-input" value="1" />';
            $msg .= '</div>';
            $msg .= '</th>';
            $msg .= '<th class="min-w-50px">Code</th>';
            $msg .= '<th class="min-w-125px">Name</th>';
            $msg .= '<th class="min-w-125px">Description</th>';
            $msg .= '<th class="min-w-125px">Payment Terms</th>';
            $msg .= '<th class="min-w-125px">Contact Person</th>';
            $msg .= '<th class="min-w-125px">Contact No</th>';
        ...
        else 
        {
            ...
            {   
                ...
                $msg .= '<td>'.$row->terms.'</td>';
                $msg .= '<td>'.$row->contact_person.'</td>';
                $msg .= '<td>'.$row->contact_no.'</td>';
        ...
        $msg .= '<div class="row"><div class="col-sm-6 pl-5"><div class="dataTables_paginate paging_simple_numbers" id="kt_supplier_table_paginate"><ul class="pagination" style="margin-bottom: 0;">';
    ...
    public function get_line_items($limit, $start_from, $keywords = '', $status)
    {
        if (!empty($keywords)) {
            ...
            ->where('suppliers.is_active', $status)
            ->where(function($q) use ($keywords) {
                $q->where('suppliers.code', 'like', '%' . $keywords . '%')
                  ->orWhere('suppliers.name', 'like', '%' . $keywords . '%')
                  ->orWhere('suppliers.description', 'like', '%' . $keywords . '%')
                  ->orWhere('suppliers.contact_person', 'like', '%' . $keywords . '%')
                  ->orWhere('suppliers.contact_no', 'like', '%' . $keywords . '%')
                  ->orWhere('payment_terms.code', 'like', '%' . $keywords . '%')
        ...
        return $res->map(function($supplier) {
            return (object) [
                'id' => $supplier->id,
                'code' => $supplier->code,
                'name' => $supplier->name,
                ...
                'modified_at' => ($supplier->updated_at !== NULL) ? date('d-M-Y', strtotime($supplier->updated_at)).'<br/>'. date('h:i A', strtotime($supplier->updated_at)) : date('d-M-Y', strtotime($supplier->created_at)).'<br/>'. date('h:i A', strtotime($supplier->created_at))
            ];
        });
    }
    ...
    {   
        ...
        $this->audit_logs('suppliers', $id, 'has removed a supplier.', Supplier::find($id), $timestamp, Auth::user()->id);
    ...
    {   
        ...
        $this->audit_logs('suppliers', $id, 'has restored a supplier.', Supplier::find($id), $timestamp, Auth::user()->id);
    ...
    {   
        $fileName = 'supplier_'.time().'.csv';

        $purchase_order_type = Supplier::select([
            'suppliers.id', 
            'suppliers.code', 
            'suppliers.name', 
            'suppliers.description',
        ...
        $columns = array('Code', 'Name', 'Description', 'Payment Terms', 'Contact Person', 'Contact No');

        $callback = function() use($purchase_order_type, $columns) {
            $file = fopen('php://output', 'w');
            fputcsv($file, $columns);

            foreach ($purchase_order_type as $supplier) {
                $row['code'] = $supplier->code;
                $row['name'] = $supplier->name;
                $row['desc'] = $supplier->description;
                $row['terms'] = $supplier->terms;
                $row['contact_person'] = $supplier->contact_person;
                $row['contact_no'] = $supplier->contact_no;
                fputcsv($file, array($row['code'], $row['name'], $row['desc'], $row['terms'], $row['contact_person'], $row['contact_no']));
            }
        ...
        {   
            ...
            if (($files = fopen($file['tmp_name'], "r")) !== FALSE) 
            {
                while (($data = fgetcsv($files, 3000, ",")) !== FALSE) 
                {
                    ...
                    if ($row > 1) 
                    {  
                        $exist = Supplier::where('code', $data[0])->get();
                        if ($exist->count() > 0) {
                            $terms = PaymentTerm::where(['name' => $data[3]])->get();
                            $supplier = Supplier::find($exist->first()->id);
                            $supplier->code = $data[0];
                            $supplier->name = $data[1];
                            $supplier->description = $data[2];
                            $supplier->payment_terms_id = ($terms->count() > 0) ? $terms->first()->id : NULL;
                            $supplier->contact_person = $data[4];
                            $supplier->contact_no = $data[5];
                            $supplier->updated_at = $timestamp;
                            $supplier->updated_by = Auth::user()->id;

                            if ($supplier->update()) {
                                $this->audit_logs('suppliers', $exist->first()->id, 'has modified a supplier.', Supplier::find($exist->first()->id), $timestamp, Auth::user()->id);
                            }
                        } else {
                            $terms = PaymentTerm::where(['name' => $data[3]])->get();
                            $supplier = Supplier::create([
                                ...
                                'name' => $data[1],
                                'description' => $data[2],
                                'payment_terms_id' => ($terms->count() > 0) ? $terms->first()->id : NULL,
