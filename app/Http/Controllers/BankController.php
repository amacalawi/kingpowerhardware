...
{   
    ...
    {   
        ...
        return view('modules/components/banks/manage')->with(compact('menus'));
    }
    ...
    {   
        ...
        return view('modules/components/banks/manage-inactive')->with(compact('menus'));
    }
    ...
    {   
        ...
        if ($rows > 0) {
            $data = array(
                'title' => 'Oh snap!',
                'text' => 'You cannot create a bank with an existing bank no.',
                'type' => 'error',
                'field' => 'code',
                'class' => 'btn-danger'
            );
        ...
        $this->audit_logs('banks', $bank->id, 'has inserted a new bank.', Bank::find($bank->id), $timestamp, Auth::user()->id);
        
        $data = array(
            'title' => 'Well done!',
            'text' => 'The bank has been successfully stored.',
            'type' => 'success',
            'class' => 'btn-brand'
        );
    ...
    {    
        ...
        if ($bank->update()) {
            $this->audit_logs('banks', $id, 'has modified a bank.', Bank::find($id), $timestamp, Auth::user()->id);
            $data = array(
                'title' => 'Well done!',
                'text' => 'The bank has been successfully modified.',
                'type' => 'success',
                'class' => 'btn-brand'
            );
            echo json_encode( $data ); exit();
    ...
    {   
        ...
        $msg .= '<table class="table align-middle table-row-dashed fs-6 gy-5" id="bankTable">';
        $msg .= '<thead>';
            ...
            $msg .= '</div>';
            $msg .= '</th>';
            $msg .= '<th class="min-w-50px">Bank No</th>';
            $msg .= '<th class="min-w-125px">Bank Name</th>';
            $msg .= '<th class="min-w-125px">Bank Account</th>';
        ...
        else 
        {
            ...
            {   
                $msg .= '<tr data-row-id="'.$row->id.'" data-row-code="'.$row->bank_no.'">';
                $msg .= '<td>';
                ...
                $msg .= '</td>';
                $msg .= '<td>';
                $msg .= '<a href="#" class="text-gray-800 text-hover-primary mb-1">'.$row->bank_no.'</a>';
                $msg .= '</td>';
                $msg .= '<td>'.$row->bank_name.'</td>';
                $msg .= '<td>'.$row->bank_account.'</td>';
    ...
    public function get_line_items($limit, $start_from, $keywords = '', $status)
    {
        if (!empty($keywords)) {
            ...
            ->where(function($q) use ($keywords) {
                $q->where('banks.bank_no', 'like', '%' . $keywords . '%')
                  ->orWhere('banks.bank_name', 'like', '%' . $keywords . '%')
                  ->orWhere('banks.bank_account', 'like', '%' . $keywords . '%');
            })
        ...
        return $res->map(function($bank) {
            return (object) [
                'id' => $bank->id,
                'bank_no' => $bank->bank_no,
                'bank_name' => $bank->bank_name,
                'bank_account' => $bank->bank_account,
                'modified_at' => ($bank->updated_at !== NULL) ? date('d-M-Y', strtotime($bank->updated_at)).'<br/>'. date('h:i A', strtotime($bank->updated_at)) : date('d-M-Y', strtotime($bank->created_at)).'<br/>'. date('h:i A', strtotime($bank->created_at))
            ];
        });
    }
    ...
    {   
        ...
        $this->audit_logs('banks', $id, 'has removed a bank.', Bank::find($id), $timestamp, Auth::user()->id);
    ...
    {   
        ...
        $this->audit_logs('banks', $id, 'has restored a bank.', Bank::find($id), $timestamp, Auth::user()->id);
    ...
    {   
        ...
        $columns = array('Bank No', 'Bank Name', 'Bank Account');

        $callback = function() use($banks, $columns) {
            $file = fopen('php://output', 'w');
            fputcsv($file, $columns);

            foreach ($banks as $bank) {
                $row['bank_no']      = $bank->bank_no;
                $row['bank_name']    = $bank->bank_name;
                $row['bank_account'] = $bank->bank_account;
                fputcsv($file, array($row['bank_no'], $row['bank_name'], $row['bank_account']));
            }
        ...
        {   
            ...
            if (($files = fopen($file['tmp_name'], "r")) !== FALSE) 
            {
                while (($data = fgetcsv($files, 3000, ",")) !== FALSE) 
                {
                    ...
                    if ($row > 1) 
                    {  
                        $exist = Bank::where('bank_no', $data[0])->get();
                        if ($exist->count() > 0) {
                            $bank = Bank::find($exist->first()->id);
                            $bank->bank_no = $data[0];
                            $bank->bank_name = $data[1];
                            $bank->bank_account = $data[2];
                            $bank->updated_at = $timestamp;
                            $bank->updated_by = Auth::user()->id;

                            if ($bank->update()) {
                                $this->audit_logs('banks', $exist->first()->id, 'has modified a bank.', Bank::find($exist->first()->id), $timestamp, Auth::user()->id);
                            }
                        } else {
                            $res = Bank::count();
                            $bank = Bank::create([
