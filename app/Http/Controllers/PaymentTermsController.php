...
{   
    ...
    {   
        ...
        return view('modules/components/payment-terms/manage')->with(compact('menus'));
    }
    ...
    {   
        ...
        return view('modules/components/payment-terms/manage-inactive')->with(compact('menus'));
    }
    ...
    {   
        ...
        if ($rows > 0) {
            $data = array(
                'title' => 'Oh snap!',
                'text' => 'You cannot create a payment term with an existing code.',
                'type' => 'error',
                'field' => 'code',
                'class' => 'btn-danger'
            );
        ...
        $this->audit_logs('payment_terms', $paymentTerm->id, 'has inserted a new payment term.', PaymentTerm::find($paymentTerm->id), $timestamp, Auth::user()->id);
        
        $data = array(
            'title' => 'Well done!',
            'text' => 'The payment term has been successfully stored.',
            'type' => 'success',
            'class' => 'btn-brand'
        );
    ...
    {    
        ...
        $paymentTerm->updated_at = $timestamp;
        $paymentTerm->updated_by = Auth::user()->id;

        if ($paymentTerm->update()) {
            $this->audit_logs('payment_terms', $id, 'has modified a payment term.', PaymentTerm::find($id), $timestamp, Auth::user()->id);
            $data = array(
                'title' => 'Well done!',
                'text' => 'The payment term has been successfully modified.',
                'type' => 'success',
                'class' => 'btn-brand'
            );
            echo json_encode( $data ); exit();
    ...
    {   
        ...
        $msg .= '<table class="table align-middle table-row-dashed fs-6 gy-5" id="paymentTermTable">';
        $msg .= '<thead>';
            ...
            $msg .= '<input class="form-check-input" type="checkbox" data-kt-check="true" data-kt-check-target="#kt_payment_terms_table .form-check-input" value="1" />';
            $msg .= '</div>';
            $msg .= '</th>';
        ...
        $msg .= '<div class="row"><div class="col-sm-6 pl-5"><div class="dataTables_paginate paging_simple_numbers" id="kt_payment_terms_table_paginate"><ul class="pagination" style="margin-bottom: 0;">';
    ...
    public function get_line_items($limit, $start_from, $keywords = '', $status)
    {
        if (!empty($keywords)) {
            ...
            ->where('payment_terms.is_active', $status)
            ->where(function($q) use ($keywords) {
                $q->where('payment_terms.code', 'like', '%' . $keywords . '%')
                  ->orWhere('payment_terms.name', 'like', '%' . $keywords . '%')
                  ->orWhere('payment_terms.description', 'like', '%' . $keywords . '%');
            })
        ...
        return $res->map(function($paymentTerm) {
            return (object) [
                'id' => $paymentTerm->id,
                'code' => $paymentTerm->code,
                'name' => $paymentTerm->name,
                'description' => $paymentTerm->description,
                'modified_at' => ($paymentTerm->updated_at !== NULL) ? date('d-M-Y', strtotime($paymentTerm->updated_at)).'<br/>'. date('h:i A', strtotime($paymentTerm->updated_at)) : date('d-M-Y', strtotime($paymentTerm->created_at)).'<br/>'. date('h:i A', strtotime($paymentTerm->created_at))
            ];
        });
    }
    ...
    {   
        ...
        $this->audit_logs('payment_terms', $id, 'has removed a payment term.', PaymentTerm::find($id), $timestamp, Auth::user()->id);
        
        $data = array(
            'title' => 'Well done!',
            'text' => 'The payment term has been successfully removed.',
            'type' => 'success',
            'class' => 'btn-brand'
        );
    ...
    {   
        ...
        $this->audit_logs('payment_terms', $id, 'has restored a payment term.', PaymentTerm::find($id), $timestamp, Auth::user()->id);
        
        $data = array(
            'title' => 'Well done!',
            'text' => 'The payment term has been successfully restored.',
            'type' => 'success',
            'class' => 'btn-brand'
        );
    ...
    {   
        $fileName = 'payment_terms_'.time().'.csv';

        $payment_terms = PaymentTerm::select([
            'payment_terms.id', 
            'payment_terms.code', 
            'payment_terms.name', 
            'payment_terms.description'
        ...
        $callback = function() use($payment_terms, $columns) {
            $file = fopen('php://output', 'w');
            fputcsv($file, $columns);

            foreach ($payment_terms as $paymentTerm) {
                $row['code']      = $paymentTerm->code;
                $row['name']      = $paymentTerm->name;
                $row['desc']      = $paymentTerm->description;
        ...
        {   
            ...
            if (($files = fopen($file['tmp_name'], "r")) !== FALSE) 
            {
                while (($data = fgetcsv($files, 3000, ",")) !== FALSE) 
                {
                    ...
                    if ($row > 1) 
                    {  
                        $exist = PaymentTerm::where('code', $data[0])->get();
                        if ($exist->count() > 0) {
                            $paymentTerm = PaymentTerm::find($exist->first()->id);
                            $paymentTerm->code = $data[0];
                            $paymentTerm->name = $data[1];
                            $paymentTerm->description = $data[2];
                            $paymentTerm->updated_at = $timestamp;
                            $paymentTerm->updated_by = Auth::user()->id;

                            if ($paymentTerm->update()) {
                                $this->audit_logs('payment_terms', $exist->first()->id, 'has modified a payment term.', PaymentTerm::find($exist->first()->id), $timestamp, Auth::user()->id);
                            }
                        } else {
                            $res = PaymentTerm::count();
                            $paymentTerm = PaymentTerm::create([
                                'code' => $data[0],
                                'name' => $data[1],
                                'description' => $data[2],
