...
var KTModalpurchase_order_typesAdd = (function () {
    ...
    return {
        init: function () {
            (i = new bootstrap.Modal(document.querySelector("#purchaseOrderTypeModal"))),
                (r = document.querySelector("#purchaseOrderType_Form")),
                (t = r.querySelector("#purchaseOrderTypeModalSubmit")),
                (e = r.querySelector("#purchaseOrderTypeModalCancel")),
                ...
                t.addEventListener("click", function (e) {
                    var formUrl = ($('#purchaseOrderTypeModal').find('input[name="method"]').val() == 'add') ? r.getAttribute('action') : base_url + 'auth/components/purchase-order-types/update/' + $('#purchaseOrderTypeModal').find('input[name="id"]').val(); 
                    var formMethod = ($('#purchaseOrderTypeModal').find('input[name="method"]').val() == 'add') ? 'POST' : 'PUT'; 
                    e.preventDefault(),
                        n &&
                                        ...
                                        ? (t.setAttribute("data-kt-indicator", "on"),
                                          ...
                                          setTimeout(function () {
                                            ...
                                            $.ajax({
                                                type: formMethod,
                                                url: formUrl,
                                                data: $('#purchaseOrderType_Form').serialize(),
                                                success: function(response) {
                                                    ...
                                                    if (data.type == 'success') {
                                                        ...
                                                        $('#purchaseOrderTypeModal').find('input[name="code"]').next().text('This is an existing code.');
                                                        Swal.fire({ title: data.title, text: data.text, icon: data.type, buttonsStyling: !1, confirmButtonText: "Ok, got it!", customClass: { confirmButton: "btn btn-primary" } }).then(
                                                            function (e) {
                                                                t.disabled = !1;
